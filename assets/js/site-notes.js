		...
		if( $notes.length > 0 ) {
			$notes.each(function() {
				...
				$note.find(".move").on("mouseup", function() {
					...
					var left_percentage = offset.left / winWidth * 100 + "%";
				...
				jQuery(window).on('resize', function () {
				    var f = $note;
				    var rightEdge = 230 + f.offset().left;
				    var screenWidth = jQuery(window).width();

			        if (rightEdge > screenWidth) {
		        		if(rzCtr == 0){
		        			prevWinSize = screenWidth;
		        			if(!$note.hasClass("collided-right")){
				        		$note.addClass("collided-right");
				        	}
		        		}
		        		else{
		        			if (screenWidth < prevWinSize) {
			        			if(!$note.hasClass("collided-right")){
					        		$note.addClass("collided-right");
					        	}
					        	rzCtr = 0;
			        ...
			        if(rightEdge >= prevWinSize &&  prevWinSize <= screenWidth){
			        	if($note.hasClass("collided-right")){
			        		$note.removeClass("collided-right");
			        		rzCtr = 1;
			        	}
			        }
